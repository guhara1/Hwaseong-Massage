...
from content.site import (BASE_URL, BRAND, BRAND_MARK, DISTRICTS, INDEXNOW_KEY,
                          NAV, PHONE, PHONE_DISPLAY, STATIONS, district_url,
                          station_url)
...
def page_priority(path: str):
    """경로 기준 sitemap 우선순위·변경 빈도를 정한다."""
    if path == "":
        return "1.0", "weekly"
    if path.endswith("-gu-chuljangmassage/"):
        return "0.8", "monthly"
    if "-station-chuljangmassage/" in path:
        return "0.8", "monthly"
    if path.startswith("hwaseong/") and path.count("/") == 3:
        return "0.7", "monthly"   # 읍·면·동
    if path.startswith("hwaseong/"):
        return "0.6", "monthly"   # 생활권·거점
    return "0.5", "monthly"       # 안내 페이지
...
<header class="site-header">
  ...
  <div class="header-top">
    <div class="header-inner">
      ...
      <p class="header-tagline"><span class="tag-gem">◆</span> 화성특례시 전지역 방문 관리 <span class="tag-gem">◆</span> 24시간 상담</p>
...
<footer class="site-footer">
  <div class="container footer-grid">
    <div class="footer-col footer-about">
      ...
      <p class="footer-desc">화성특례시 전지역 방문 출장마사지·홈타이 안내 사이트입니다. 모든 서비스는 안내된 관리 범위와 위생·안전 기준 안에서만 제공됩니다.</p>
      <address class="footer-contact">
        ...
        <span class="footer-contact-row"><span class="footer-label">서비스 지역</span> 화성특례시 전지역</span>
      </address>
    </div>
    <nav class="footer-col" aria-label="행정구 안내">
      <p class="footer-title">행정구 안내</p>
      <ul>
        {''.join(f'<li><a href="{district_url(slug)}">{name} 출장마사지</a></li>' for slug, name in DISTRICTS)}
      </ul>
    </nav>
    <nav class="footer-col" aria-label="지하철역 안내">
      <p class="footer-title">역세권 안내</p>
      <ul>
        {''.join(f'<li><a href="{station_url(slug)}">{name}</a></li>' for slug, name in STATIONS[:5])}
      </ul>
    </nav>
  ...
  <div class="footer-bottom">
    <div class="container footer-bottom-inner">
      ...
      <p class="footer-note">화성특례시 전지역을 안내하는 건전한 방문 관리 사이트이며, 불법적인 요청은 어떤 경우에도 응하지 않습니다.</p>
...
def build() -> None:
    ...
    for page in PAGES:
        ...
        if not noindex:
            sitemap_urls.append(
                (BASE_URL.rstrip("/") + "/" + path, page["title"], page["desc"])
            )
    ...
    rows = []
    for loc, _title, _desc in sitemap_urls:
        path = loc[len(base) + 1:]
        prio, freq = page_priority(path)
        rows.append(
    ...
    # rss.xml — 색인 가속용 RSS 2.0 피드 (구글·빙 사이트맵으로도 제출 가능)
    pubdate = format_datetime(now)
    items = []
    for loc, title, desc in sitemap_urls:
        items.append(
            "    <item>\n"
    ...
    with open(os.path.join(ROOT, "rss.xml"), "w", encoding="utf-8") as f:
        f.write(
            ...
            "  <channel>\n"
            f"    <title>{html.escape(BRAND)} 화성 출장마사지·홈타이 안내</title>\n"
            f"    <link>{base}/</link>\n"
            f'    <atom:link href="{base}/rss.xml" rel="self" type="application/rss+xml"/>\n'
            "    <description>화성특례시 전지역 방문 출장마사지·홈타이 지역별 예약 안내</description>\n"
    ...
    # IndexNow 키 파일 — 루트에 게시되어야 빙·네이버가 소유권을 확인한다.
